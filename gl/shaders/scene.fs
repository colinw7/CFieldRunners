...
vec3 calcNormal() {
  if (normalTexture.enabled) {
    vec3 norm = texture(normalTexture.texture, TexCoords + textureOffset).rgb;
...
vec3 calcDiffuseColor() {
  if (diffuseTexture.enabled) {
    vec4 textureColor = texture(diffuseTexture.texture, TexCoords + textureOffset);
...
vec3 calcSpecularColor() {
  if (specularTexture.enabled)
    return texture(specularTexture.texture, TexCoords + textureOffset).rgb;
  else
...
vec3 calcEmissionColor() {
  if (emissiveTexture.enabled)
    return texture(emissiveTexture.texture, TexCoords + textureOffset).rgb;
  else
...
void main() {
  ...
  //FragColor = vec4(diffFactor, diffFactor, diffFactor, 1.0);
